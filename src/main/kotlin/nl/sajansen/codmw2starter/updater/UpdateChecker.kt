...
import nl.sajansen.codmw2starter.gui.updater.UpdatePopup
...
class UpdateChecker(private val urlProvider: wURL = wURL()) {
    ...
    fun getLatestVersion(): String? {
        ...
        return try {
            ...
            logger.error("Failed to parse version JSON response: '${jsonResponse}'")
            e.printStackTrace()
            Notifications.add(
            ...
            logger.error("Failed to retrieve latest application version: invalid response")
            t.printStackTrace()
            Notifications.add(
    ...
    fun getLatestVersionResponse(): String? {
        return try {
            ...
            logger.error("Failed to retrieve latest application version: invalid URL: '${ApplicationInfo.latestVersionsUrl}'")
            e.printStackTrace()
            Notifications.add(
